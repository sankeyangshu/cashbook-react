...
import { getBillTypeListAPI, addBillAPI } from '@/api/bill';
...
const BillPopupAdd = forwardRef<billAddRefType, billPopupAddType>(({ onReload }, ref) => {
        ...
        <div className={mCss['type-warp']}>
          <div className={mCss['type-body']}>
            {(payType === 'expense' ? expense : income).map((item) => (
              <div onClick={() => setCurrentType(item)} key={item.id} className={mCss['type-item']}>
                <span
